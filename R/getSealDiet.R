...
getSealDiet <- function(params , n = params@initial_n , n_seal = params@initial_n_other[['seals']], idx_sp = 1:length(params@w) , t = NULL , ...) {
  ...
  seal_interaction <- seal_params$interaction_seal
  ...
  for(i in 1:nrow(diet)) {
    diet[i,] = diet[i,]*encounterSearchVol$search_vol*seal_params$interaction_seal[i]
  }
