...
mizerTMortGear <- function (params , effort) {
  out <- params@trawlParams$selectivity
  out[] <- effort * c(params@trawlParams$catchability) * c(params@trawlParams$selectivity)
  return(out)
}
...
getTrawlMort <- function (object, effort, time_range) {
  if (is(object, "MizerSim")) {
    ...
    if (nrow(sim@params@trawlParams$effort) != nrow(sim@effort)) stop('Trawl effort must be same length as sim effort.')
    time_elements <- get_time_elements(sim, time_range)
    t_mort_gear <- getTrawlMort(sim@params, sim@params@trawlParams$trawl_effort)
    return(t_mort_gear[time_elements, , , , drop = FALSE])
  }
  else {
    params <- validParams(object)
    if (missing(effort)) {
      effort <- rep(1 , dim(params@trawlParams$selectivity)[1])
    }
    if (is(effort, "numeric")) {
      no_gear <- dim(params@trawlParams$catchability)[1]
      if (length(effort) == 1) {
      ...
      dimnames(f) <- dimnames(params@trawlParams$selectivity)
      return(f)
    }
    else {
      no_gear <- dim(params@trawlParams$catchability)[1]
      ...
      out <- array(NA, dim = c(dim(params@trawlParams$selectivity), 
                               dim(effort)[1]), dimnames = c(dimnames(params@trawlParams$selectivity), 
