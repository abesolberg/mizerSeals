...
names(params@trawl_params) <- paste0(names(params@trawl_params) , '_trawl')

dat <- append(dat , params@trawl_params)
...
fun <- function(...) {
  ...
  # Fit to seal biomass
  log_diff_s <- log(rowSums(s_at_y*params@seal_params$dw)) - log(rep(obs$seals , years))
...
data <- list(
  ...
  alpha_seal = params@seal_params$alpha ,
  H = params@seal_params$sealHarvest ,
...
pars <- list(
  rdd = params@other_params$rdd ,
  z = params@other_params$z ,
  catchability_trawl = params@trawl_params$catchability,
  M_seal = params@seal_params$M ,
  resource_interaction_seal = params@seal_params$resource_interaction_seal ,
